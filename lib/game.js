...
class Game {
  ...
  newBlockSet() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      block.initialPos = this.blockSetPos[i].pos;
  ...
  validMove(x, y, block) {
    if (x > this.board.grid.length - 1 || y > this.board.grid.length - 1) {
      return false;
    }
    ...
    for (let i = 0; i < block.length; i++) {
      for (let j = 0; j < block[i].length; j++) {
        ...
        if (tile.full) {
          ...
          if (!relTile.gridPos || !relTile.validGrid || relTile.full) {
            return false;
          }
        }
      }
  ...
  toggleMoving(mouseX, mouseY, isMoving) {
    this.blockSet.forEach((block) => {
      if (block.includesPos(mouseX, mouseY)) {
        if (!isMoving) {
          let midX = Math.ceil((mouseX - 250) / 50);
          let midY = Math.ceil((mouseY - 250) / 50);
